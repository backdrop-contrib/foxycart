...
function foxycart_update_backdrop_user($fc_customer_id, $fc_email, $fc_password, $user_id = NULL) {
  ...
  if (!$user) {
    foxycart_log("foxycart_update_backdrop_user: Did not find user by user_id");
    // If we didn't find it try to load by email address
    $user = user_load_by_mail($fc_email);
    if (!$user) {
      foxycart_log("foxycart_update_backdrop_user: Did not find user by email address");
      ...
      foxycart_log("foxycart_update_backdrop_user: Created user: " . $user->uid);
    } else {
      foxycart_log("foxycart_update_backdrop_user: Found user by email");
    }
  } else {
    foxycart_log("foxycart_update_backdrop_user: Found user by user_id");
  }
...
function foxycart_process_sso() {
  ...
  backdrop_add_http_header('Location', $full_redirect);
}
?>
